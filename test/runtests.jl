...
@testset "MetXCultureHub.jl" begin
    ...
    # ===============================
    let
        citekey = "rathCharacterisationCellGrowth2017"
        culdb = pull_cul(citekey)
        
        ## -------------------------------------
        # checking ststs
        iters = query(culdb, "ROOT", "Iters")
        for stst in iters["stst_ids"]
            # q_glc = c_glc * D / X (Assuming glucose limitation)
            c_glc = query(culdb, "ROOT", "apiid" => "c_glc", "culid" => stst)["val"]
            D = query(culdb, "ROOT", "apiid" => "D", "culid" => stst)["val"]
            X = query(culdb, "ROOT", "apiid" => "Xv", "culid" => stst)["val"]
            q_glc = query(culdb, "ROOT", "apiid" => "q_glc", "culid" => stst)["val"]
            
            @test isapprox(abs(c_glc * D / X), abs(q_glc); rtol = 12/100)
        end
    end
    
    # ===============================
    # TODO: up folsomPhysiologicalBiomassElemental2015 to ContextDBs
    # let
